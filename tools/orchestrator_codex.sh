...
- FRAME-SIZE / STACK-OFFSET residue (the usual big-func wall): if the ONLY diffs are the prologue/epilogue \`addiu sp,sp,N\` (wrong frame size) and/or a UNIFORM shift in sp-relative load/store offsets, your stack LOCALS are mis-sized -- this is NOT a logic bug, do NOT rewrite the body. Fixes: (a) re-derive each stack local's exact size from the CALLEE store widths writing it (sh->s16, swc1->f32, sb->u8, sw->s32); size buffers to the EXACT bytes used (an oversized array pads to the next 8-aligned slot and shifts everything); (b) to SHRINK the frame, cut simultaneously-live temps (reuse one local, inline an expression) so IDO allocates fewer spill slots; (c) if the frame is larger by a fixed pad that does NOT shift any offset, it's spill-slot packing -- tune with a \`volatile s32 pad[N];\` (small N) or remove an over-declared local. Attack the frame NUMBER directly.
